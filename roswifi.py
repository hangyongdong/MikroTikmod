...
def patch_npk_multiple(input_npk, src_dirs, output_npks, target_path):
    ...
    try:
        ...
        for i, (s_dir, o_npk) in enumerate(zip(src_dirs, output_npks)):
            ...
            # 防呆设计：去除目标路径开头的斜杠，确保其被安全解析为 NPK 内部的相对路径
...
def main():
    ...
    # 🌟 优化：保持 -b/--bdwlan 兼容，同时增加 --src 别名，使其在修改普通 NPK 时语义更合理
    parser.add_argument(
        "-b",
        "--bdwlan",
        "--src",
    ...
    # 🌟 新增：允许指定 NPK 内部的目标路径，默认值为 lib/bdwlan 以保持旧工作流无缝兼容
    parser.add_argument(
        "-t",
        "--target",
        default="lib/bdwlan",
