...
# 自定义镜像站域名
MIRROR_HOST="https://routeros.ydhang.cn"
...
select_language() {
    while true; do
        ...
        echo "1. English"
        echo "2. 简体中文"
        ask_until "Please choose an option" "2"
        case $resp in
            1) 
                MSG_ARCH="Arch:"
    ...
    case $ARCH in
        x86_64|i386|i486|i586|i686)
            if [ "$BOOT_MODE" == "BIOS" ]; then
                IMG_NAME="chr-${VERSION}-x86-legacy-bios.img.zip"
            else
                IMG_NAME="chr-${VERSION}-x86-uefi.img.zip"
            fi
            IMG_URL="${MIRROR_HOST}/extra/${VERSION}/${IMG_NAME}"
            ;; 
        aarch64|arm64)
            IMG_NAME="chr-${VERSION}-arm64.img.zip"
            IMG_URL="${MIRROR_HOST}/extra/${VERSION}/${IMG_NAME}"
            ;; 
        *)
